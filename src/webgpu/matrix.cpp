...
Matrix Matrix::add(const Matrix& other) const {
    ...
    bool webgpuAvailable = EM_ASM_INT({
        return (typeof window.gpuDevice !== 'undefined' && window.gpuDevice !== null) ? 1 : 0;
    });
    ...
    EM_ASM({
        try {
            ...
            commandEncoder.copyBufferToBuffer(bufferC, 0, readbackBuffer, 0, dataSize);
            ...
            readbackBuffer.mapAsync(GPUMapMode.READ).then(() => {
                ...
                const resultHeap = new Float32Array(HEAPF32.buffer, ptrResult, rows * cols);
...
extern "C" {
    EMSCRIPTEN_KEEPALIVE
    void matrix_addCPU_fallback(float* a, float* b, float* result, size_t rows, size_t cols) {
